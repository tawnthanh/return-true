...
def request_exists_except_current(form, field):
    title = field.data
    user = current_user.to_dict()
    request = Request.query.filter(Request.title == form.data["title"], Request.userId == user["id"], Request.id != form.data["id"]).first()
    if request:
        raise ValidationError("This request already exist")
...
class RequestEditForm(FlaskForm):
    ...
    title = StringField("title", validators=[DataRequired(), request_exists_except_current])
